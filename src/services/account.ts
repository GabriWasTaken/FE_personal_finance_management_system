...
export const deleteAccount = async ( { id, handleError }: { id: number, handleError: ({ error }: { error: { status: number } }) => void } ) => {
  const response = await fetch(import.meta.env.VITE_BASE_URL + '/accounts?id=' + id, {
    method: 'DELETE',
    headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` }
  });
  if (!response.ok) {
...
export const deleteSubcategory = async ( { id, handleError }: { id: number, handleError: ({ error }: { error: { status: number } }) => void } ) => {
  const response = await fetch(import.meta.env.VITE_BASE_URL + '/subcategories?id=' + id, {
    method: 'DELETE',
    headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` }
  });
  if (!response.ok) {
